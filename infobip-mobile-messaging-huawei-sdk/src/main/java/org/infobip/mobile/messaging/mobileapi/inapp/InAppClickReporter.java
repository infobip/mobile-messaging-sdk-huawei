...
public class InAppClickReporter {
    ...
    private @NonNull Map<String, String> getHeaders(String[] payload) {
        ...
        if (mobileMessagingCore.getApplicationCode() != null) {
            headers.put("Authorization", "App " + mobileMessagingCore.getApplicationCode());
            headers.put("pushregistrationid", mobileMessagingCore.getPushRegistrationId());
            headers.put("buttonidx", payload[1]);
            headers.put("User-Agent", payload[2]);
        }
        return headers;
    }
